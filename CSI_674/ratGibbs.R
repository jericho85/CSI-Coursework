...
for (k in 2:numSim) {
	...
	vMC[k] <- sample(vGrid,1,prob=vPost)  # choose value of V
	}
...
traceplot(as.mcmc(uMC))
traceplot(as.mcmc(vMC))
...
# Plot amount of shrinkage against sample size
diff <- nt/nr - qMean
...
f=nt/nr   # Frequency
qunif05=qbeta(0.05,nt+1,nr-nt+1)
qunif95=qbeta(0.95,nt+1,nr-nt+1)
u=(nt+1)/(nr+2)
plot(f,u,pch=20,col="red",ylim=c(0,.55),xlab="Sample Frequency",ylab="Tumor Probability")
lines(f[order(f)],u[order(f)],col="red",lty=1)
lines(f[order(f)],qunif95[order(f)],lty=3,col="red")
lines(f[order(f)],qunif05[order(f)],lty=3,col="red")
...
lines(f[order(f)],q95[order(f)],lty=2,col="blue")
lines(f[order(f)],q05[order(f)],lty=2,col="blue")
legend(x=0,y=0.51,legend=c("Pooled Uniform","Hierarchical Bayes"),lty=c(3,2),col=c("red","blue"))
