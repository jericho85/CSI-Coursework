...
# The data
library(UsingR)   # baby birth data is available in UsingR package
birthtimes=babyboom$running.time
n = length(birthtimes)                      # number of births
sum.ibt = birthtimes[length(birthtimes)]/60 # sum of inter-birth times transformed to hours
...
beta0 = 0.75      # Prior inverse-scale (scale of the Gamma distribution for birth rate)

alpha1 = 2 + 44   # posterior shape
beta1 = (1/beta0 + sum.ibt)^-1  # posterior inverse-scale
